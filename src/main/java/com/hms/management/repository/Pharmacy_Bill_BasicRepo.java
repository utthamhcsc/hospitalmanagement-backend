...
//		+ "u.name as patientName,u.userId as patientId "
//		+ "from Pharmacy_Bill_Basic b left join UserRegistration d on d.userId=b.doctor "
//		+ "left join UserRegistration u on u.userId=b.patientId "
//		+ "where SUBSTRING(b.date,1,10) between SUBSTRING(?1,1,10) AND SUBSTRING(?2,1,10)")	

	@Query("select p from Pharmacy_Bill_Basic p where SUBSTRING(p.date,1,10) between SUBSTRING(?1,1,10) and SUBSTRING(?2,1,10)")
	List<Pharmacy_Bill_Basic> fetchBillByDate(String fromDate,String toDate);
